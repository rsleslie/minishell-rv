...
/*   Updated: 2023/06/12 21:25:47 by rleslie-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	norm_executo_pipe(t_exec*aux, t_config *data, pid_t *pid)
{
	if (op_builtins(aux->cmd[0]) != 0)
	{
		free(pid);
		ft_free_tab_int(data->fd_pipe, pipe_counter(data->tokens));
		free_var(data->node_env, data->node_export,
			data, data->node_exec);
...
void	executor_pipe(t_exec *exec, t_config *data, t_node *env, t_node *export)
{
	...
	while (++vars.i <= aux->index)
	{
		...
		if (pid[vars.i] == 0)
		{
			...
			norm_executo_pipe(aux, data, pid);
		}
		if (aux->next != NULL)
			aux = aux->next;
	}
